-- Create test data for attendance system with correct age_group values

-- First, let's see what age_group values are allowed
-- Let's create some sample classes with valid age_group values
...
(gen_random_uuid(), 'Beginner Karate', 'Introduction to Karate for beginners', 
 (SELECT id FROM profiles WHERE role IN ('instructor', 'admin', 'owner') LIMIT 1), 15, 60, 'all', 'beginner', true),
(gen_random_uuid(), 'Advanced BJJ', 'Brazilian Jiu-Jitsu for advanced students', 
 (SELECT id FROM profiles WHERE role IN ('instructor', 'admin', 'owner') LIMIT 1), 12, 90, 'all', 'advanced', true),
(gen_random_uuid(), 'Youth Taekwondo', 'Taekwondo fundamentals for youth', 
 (SELECT id FROM profiles WHERE role IN ('instructor', 'admin', 'owner') LIMIT 1), 20, 45, 'all', 'intermediate', true);
