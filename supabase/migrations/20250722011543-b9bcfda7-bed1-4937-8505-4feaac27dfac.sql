-- Phase 1: Multi-Academy Database Schema Changes (Data Cleanup)
...
-- 2. First, normalize existing role data
UPDATE public.profiles SET role = 'student' WHERE role = 'member';

-- 3. Clean up orphaned profile records (profiles without corresponding auth.users)
DELETE FROM public.profiles 
WHERE id NOT IN (SELECT id FROM auth.users);

-- 4. Create many-to-many relationship table for user-academy memberships
...
-- 5. Create academy switching audit log
...
-- 6. Migrate existing user-academy relationships to new table (only valid profiles)
INSERT INTO public.academy_memberships (user_id, academy_id, role, joined_at, is_active)
SELECT p.id, p.academy_id, p.role, p.created_at, true
FROM public.profiles p
INNER JOIN auth.users u ON p.id = u.id
WHERE p.academy_id IS NOT NULL
ON CONFLICT (user_id, academy_id) DO NOTHING;

-- 7. Set last_academy_id to current academy_id for existing users
...
-- 10. Create RLS policies for academy_memberships (CRITICAL for data isolation)
...
-- 11. Create RLS policies for academy_switches (audit trail)
...
-- 12. Create helper functions for multi-academy access control
